...
public class ContabilUsuarioController {
	...
	public ResponseEntity<ResponseDto<List<ContabilUsuarioDto>>> findAll () throws Exception{
		return ResponseEntity
			.ok(new ResponseDto<List<ContabilUsuarioDto>>(service.findAll(), null));
	}
	
	@GetMapping("/{clerkId}/")
	public ResponseEntity<ResponseDto<ContabilUsuarioDto>> findByClerkId (@PathVariable("clerkId") String clerkId) throws Exception {
		return ResponseEntity
			.ok(new ResponseDto<ContabilUsuarioDto>(service.findByClerkId(clerkId), null));
	}
	
	@PutMapping("/{clerkId}/{hearts}/{points}")
	public ResponseEntity<ResponseDto<Integer>> update(@PathVariable("clerkId") String clerkId, @PathVariable("hearts") int hearts, @PathVariable("points") int points) throws Exception {		
		return ResponseEntity
